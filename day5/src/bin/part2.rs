...
fn part1(input: &str) -> u32 {
    if let Ok((updates_str, rules)) = parse_rules(input) {
        ...
        // println!("rules {:?}", rules);
        if let Ok((_remain, mut updates)) = parse_updates(updates_str) {
            ...
            return updates
                ...
                .filter_map(|update| {
                    ...
                    'correction_loop: loop {
                        for (l, r) in &rules {
                            ...
                            match (l_pos, r_pos) {
                                (Some(l_index), Some(r_index)) => {
                                    debug_assert!(l_index != r_index);
                                    if r_index < l_index {
                                        failure_count += 1;
                                        // Correct : L must come before R.
                                        correction_count += 1;
                                        if correction_count > 99 {
                                            panic!("too many corrections.")
                                        }
                                        update.swap(l_index, r_index);
                                        //(Restart rule checking)
                                        continue 'correction_loop;
                                    }
                                }
                                _ => {
                        ...
                        // No rule issued a correction.
                        break 'correction_loop;
                    }
                    ...
                    if failure_count > 0 {
                        // if failure_count > 1 {
                        // dbg!(failure_count);
                        // panic!("more than one correction");
                        // }
                        Some(update)
                    } else {
                        None
